    ...
    <div class='max-w-screen-xl m-auto'>
      ...
      <div class='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 auto-rows-fr gap-4 sm:gap-1 lg:gap-1 px-8 snap-y snap-mandatory'>
        {#each asa.members as member} 
          <div role='button' tabindex="0" data-card="front" data-name="{member.name}" class='snap-start bg-asa-red relative rounded-md overflow-hidden duration-100' on:click={handleClick} on:mouseover={handleMouseOver} on:mouseleave={handleMouseLeave} on:focus>
          ...
          <div role='button' tabindex="0" data-card="back" data-name="{member.name}" class='overflow-y-auto relative member-detail hidden rounded-md overflow-hidden duration-100' on:click={handleClick}>
            {#if member.imgs !== undefined}
